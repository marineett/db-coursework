...
// DuckDB helpers
// GetDuckDBConnectionString returns a DSN for DuckDB.
// - If DUCKDB_PATH is empty, it uses in-memory (":memory:").
// - If DUCKDB_PATH starts with "file:" or equals ":memory:", it's used as-is.
// - Otherwise, it prefixes with "file:" (persistent file database).
func GetDuckDBConnectionString() string {
...
func CreateDuckDBConnection(connectionString string) (*sql.DB, error) {
	log.Println("Attempting to connect to DuckDB...")
	...
	// DuckDB is embedded; Ping still validates basic initialization.
	if err := db.Ping(); err != nil {
		log.Printf("Error pinging DuckDB: %v", err)
		return nil, fmt.Errorf("error connecting to DuckDB: %v", err)
	}
	log.Println("Successfully opened DuckDB!")
	return db, nil
}
