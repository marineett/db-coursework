...
const ClientPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            ...
            const response = await fetch(`/api/client/get_profile?id=${clientId}&reviews_offset=${offset}&reviews_limit=${REVIEWS_PER_BATCH}`);
            if (response.ok) {
        ...
        <Layout style={{ minHeight: '100vh' }}>
            ...
            <Content style={{ padding: '24px', marginTop: 64 }}>
                <div style={{ padding: 24, background: '#fff', minHeight: 360 }}>
                    <ChatListWindow fetchUrl="/api/chat/get_client_chats" userId={clientId} />
                    {profile && (
