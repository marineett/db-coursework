...
const ModeratorList: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const moderatorsResponse = await fetch('/api/admin/get_moderators?id=' + localStorage.getItem('user_id'), {
                    headers: {
                ...
                const departmentsResponse = await fetch(`/api/admin/get_departments?id=${adminId}`, {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('token')}`
                    }
                });
    ...
    const handleHire = async (moderatorId: number, departmentId: number) => {
        try {
            const adminId = localStorage.getItem('user_id');
            const url = `/api/admin/hire_moderator?id=${adminId}&d_id=${departmentId}&m_id=${moderatorId}`;
