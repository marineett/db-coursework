...
const ContractCard: React.FC<ContractCardProps> = ({ contract, mode, onRespond, repetitorId, onSelect }) => {
    ...
    const fetchLessons = async () => {
        ...
        try {
            const response = await fetch(`/api/contract/get_lessons?contract_id=${contract.id}&lessons_offset=${lessonsOffset}&lessons_size=${lessonsSize}`);
    ...
    const handleAddLesson = async (values: { duration: number }) => {
        ...
        try {
            const response = await fetch('/api/contract/add_lesson', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                ...
                <div className="mt-6 flex flex-col gap-2">
                    <button
                        onClick={async (e) => {
                            ...
                            try {
                                const url = `/api/repetitor/cancel_contract?id=${repetitorId}&c_id=${contract.id}`;
                                const response = await fetch(url, {
                                    method: 'GET',
                                    credentials: 'include',
                                });
