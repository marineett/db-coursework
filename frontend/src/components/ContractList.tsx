...
const ContractList: React.FC = () => {
    ...
    const fetchContracts = async (from: number) => {
        try {
            const response = await fetch(`/api/moderator/get_contracts?from=${from}&size=${BATCH_SIZE}`, {
                headers: {
    ...
    const handleBanContract = async (contractId: number) => {
        ...
        try {
            const response = await fetch(`/api/moderator/ban_contract?id=${contractId}`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
                }
    ...
    const handleOpenCRChat = async (clientId: number, repetitorId: number) => {
        try {
            const res = await fetch(API_ENDPOINTS.CHAT.START_CR_CHAT(clientId, repetitorId), {
                method: 'GET',
            ...
            alert('Не удалось открыть чат клиент-релетитор');
        }
    };

    const handleOpenCMChat = async (clientId: number) => {
        try {
            const res = await fetch(`/api/chat/start_cm_chat?c_id=${clientId}`, {
                method: 'GET',
                headers: { 'Accept': 'application/json' },
                credentials: 'include',
            });
    ...
    const handleOpenRMChat = async (repetitorId: number) => {
        try {
            const res = await fetch(`/api/chat/start_rm_chat?r_id=${repetitorId}`, {
                method: 'GET',
                headers: { 'Accept': 'application/json' },
                credentials: 'include',
            });
                                            ...
                                            <button
                                                onClick={async () => {
                                                    ...
                                                    if (contract.client_id && moderatorId) {
                                                        try {
                                                            const res = await fetch(`/api/chat/start_cm_chat?c_id=${contract.client_id}&m_id=${moderatorId}`, {
                                                                method: 'GET',
                                                                headers: { 'Accept': 'application/json' },
                                                                credentials: 'include',
                                                            });
                                            ...
                                            <button
                                                onClick={async () => {
                                                    ...
                                                    if (contract.repetitor_id && moderatorId) {
                                                        try {
                                                            const res = await fetch(`/api/chat/start_rm_chat?r_id=${contract.repetitor_id}&m_id=${moderatorId}`, {
                                                                method: 'GET',
                                                                headers: { 'Accept': 'application/json' },
                                                                credentials: 'include',
                                                            });
