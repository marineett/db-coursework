...
func PrintRepetitorData(repetitorData *types.RepetitorData) {
	fmt.Println("Repetitor info:")
	fmt.Println("ID:", repetitorData.ID)
	fmt.Println("Personal data ID:", repetitorData.PersonalDataID)
	fmt.Println("Registration date:", repetitorData.RegistrationDate)
}

func PrintRepetitorInfo(repetitorID int64, serviceModule *service_logic.ServiceModule) {
	fmt.Println("Repetitor info:")
	repetitorData, err := serviceModule.RepetitorService.GetRepetitorData(repetitorID)
	if err != nil {
		fmt.Println("Error:", err)
		return
	}
	personalData, err := serviceModule.PersonalDataService.GetPersonalData(repetitorData.PersonalDataID)
	if err != nil {
		fmt.Println("Error:", err)
		return
	}
