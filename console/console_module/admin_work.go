...
func PrintAdminData(adminData *types.AdminData) {
	fmt.Println("Admin info:")
	fmt.Println("ID:", adminData.ID)
	fmt.Println("Personal data ID:", adminData.PersonalDataID)
	fmt.Println("Registration date:", adminData.RegistrationDate)
	fmt.Println("Department ID:", adminData.DepartmentID)
	fmt.Println("Salary:", adminData.Salary)
}

func PrintAdminInfo(adminID int64, serviceModule *service_logic.ServiceModule) {
	fmt.Println("Admin info:")
	adminData, err := serviceModule.AdminService.GetAdminData(adminID)
	if err != nil {
		fmt.Println("Error:", err)
		return
	}
	personalData, err := serviceModule.PersonalDataService.GetPersonalData(adminData.PersonalDataID)
	if err != nil {
		fmt.Println("Error:", err)
		return
	}
